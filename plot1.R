...
household_data_consumption <- read.table(file = filename, header = TRUE, sep = ";",  na.strings = "?")

#Apply as.Date(). The date format is %d/%m%/%Y

household_data_consumption$Date <- as.Date(household_data_consumption$Date, "%d/%m/%Y")

#Subset the data between 2007-02-01 and 2007-02-02

HDC_subset <- subset(household_data_consumption, Date == "2007-02-01" | Date == "2007-02-02")

#remove the original dataset to free RAM

rm(household_data_consumption)
